...
useKeyboardShortcuts([
  ...
  { key: 'ArrowLeft', handler: () => seekRelative(-10), description: 'Seek back 10s' },
  { key: 'ArrowRight', handler: () => seekRelative(10), description: 'Seek forward 10s' },
  { key: 'ArrowUp', handler: () => setSpeed(s => Math.min(s * 2, 16)), description: 'Speed up' },
  { key: 'ArrowDown', handler: () => setSpeed(s => Math.max(s / 2, 0.25)), description: 'Slow down' },
])
...
```typescript
const [searchCapability, setSearchCapability] = useState<'full' | 'basic' | 'loading'>('loading')

useEffect(() => {
  apiFetch('/system/info').then(res => res.json()).then(data => {
...
{searchCapability === 'basic' && (
  <p className="text-xs text-amber-400 mt-1">
    Basic search only (class name matching). Install CLIP models for natural language search.
  </p>
)}
```
...
```tsx
{discovering && (
  <button onClick={() => discoverController?.abort()} className="...">Cancel</button>
)}
```
...
```tsx
<p className="text-xs text-nvr-text-muted mt-1">
  How many days to keep recordings before automatic deletion. Higher values use more storage.
</p>
```
...
const handleExportAudit = () => {
  ...
  const rows = auditEntries.map(e => [
    e.created_at, e.username, e.action, `${e.resource_type}/${e.resource_id}`, e.details, e.ip_address
  ])
...
Button:
```tsx
<button onClick={handleExportAudit} className="text-xs text-nvr-accent hover:text-nvr-accent-hover">
  Export CSV
</button>
```
...
function ChangePasswordForm() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (res.ok) { setSuccess(true); setCurrentPw(''); setNewPw(''); setConfirmPw('') }
...
```typescript
const errorMessage = error.includes('timeout') || error.includes('network')
