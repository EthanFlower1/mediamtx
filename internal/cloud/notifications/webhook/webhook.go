...
func (a *Adapter) doSend(ctx context.Context, msgID string, body []byte) (notifications.CommsDeliveryResult, string) {
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, a.url, bytes.NewReader(body))
	if err != nil {
		return notifications.CommsDeliveryResult{}, fmt.Sprintf("new request: %v", err)
	}
	...
	if err != nil {
		return notifications.CommsDeliveryResult{}, fmt.Sprintf("post: %v", err)
	}
	defer resp.Body.Close()
	io.Copy(io.Discard, resp.Body) //nolint:errcheck

	if resp.StatusCode >= 500 {
		return notifications.CommsDeliveryResult{}, fmt.Sprintf("server error HTTP %d", resp.StatusCode)
	}
