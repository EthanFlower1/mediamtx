...
func TestSend_NoSecret(t *testing.T) {
	...
	result := adapter.Send(context.Background(), notifications.Message{ID: "msg-2"})
	if result.State != notifications.DeliverySuccess {
...
func TestSend_RetryOnServerError(t *testing.T) {
	...
	result := adapter.Send(context.Background(), notifications.Message{ID: "msg-3"})
	if result.State != notifications.DeliverySuccess {
...
func TestSend_AllRetriesFail(t *testing.T) {
	...
	result := adapter.Send(context.Background(), notifications.Message{ID: "msg-5"})
	if result.State != notifications.DeliveryFailure {
		t.Fatalf("expected failure, got %s", result.State)
	}
}
