...
func (d *DB) DeleteFederation(id string) error {
	_, err := d.Exec(`DELETE FROM federation_peers WHERE federation_id = ?`, id)
	if err != nil {
		return fmt.Errorf("delete federation peers: %w", err)
	}
	_, err = d.Exec(`DELETE FROM federations WHERE id = ?`, id)
	if err != nil {
...
func (d *DB) RemoveFederationPeer(id string) error {
	_, err := d.Exec(`DELETE FROM federation_peers WHERE id = ?`, id)
	if err != nil {
