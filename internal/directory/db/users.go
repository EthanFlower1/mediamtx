...
func (d *DB) CreateUser(u *User) error {
	...
	_, err := d.Exec(`
		INSERT INTO users (id, username, password_hash, role_id, camera_permissions, locked_until, failed_login_attempts, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		u.ID, u.Username, u.PasswordHash, u.RoleID, u.CameraPermissions,
...
func (d *DB) GetUser(id string) (*User, error) {
	u := &User{}
	err := d.QueryRow(`
		SELECT id, username, password_hash, role_id, camera_permissions, locked_until, failed_login_attempts, created_at, updated_at
		FROM users WHERE id = ?`, id,
	).Scan(&u.ID, &u.Username, &u.PasswordHash, &u.RoleID, &u.CameraPermissions,
		&u.LockedUntil, &u.FailedLoginAttempts, &u.CreatedAt, &u.UpdatedAt)
...
func (d *DB) UpdateUser(u *User) error {
	...
	res, err := d.Exec(`
		...
		WHERE id = ?`,
		u.Username, u.PasswordHash, u.RoleID, u.CameraPermissions,
