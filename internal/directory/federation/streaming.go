...
// Ensure StreamingHandler implements the connect handler interface at compile time.
var _ kaivuev1connect.FederationPeerServiceHandler = (*StreamingHandler)(nil)
...
// StreamingHandler implements the FederationPeerService streaming RPCs
// (SearchRecordings, MintStreamURL).
...
// StreamingHandlerConfig configures StreamingHandler construction.
...
const (
	defaultStreamingMaxTTL   = 5 * time.Minute
	defaultStreamingPageSize = 100
	maxStreamingPageSize     = 1000
)

// NewStreamingHandler creates a federation streaming handler.
func NewStreamingHandler(cfg StreamingHandlerConfig) (*StreamingHandler, error) {
	if cfg.Enforcer == nil {
...
func (h *StreamingHandler) SearchRecordings(
	ctx context.Context,
	...
	if pageSize <= 0 {
		pageSize = defaultStreamingPageSize
	}
	if pageSize > maxStreamingPageSize {
		pageSize = maxStreamingPageSize
	}
...
// connect to directly.
func (h *StreamingHandler) MintStreamURL(
	ctx context.Context,
