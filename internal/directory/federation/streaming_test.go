...
const (
	streamTestPeerID   = "peer-dir-1"
	streamTestTenantID = "tenant-A"
	streamTestCamID    = "cam-1"
	streamTestRecID    = "rec-1"
	streamTestRecURL   = "https://recorder.example.com"
)

var streamTestTenant = &kaivuev1.TenantRef{
	Type: kaivuev1.TenantType_TENANT_TYPE_CUSTOMER,
	Id:   streamTestTenantID,
}

var fixedNow = time.Date(2026, 4, 10, 12, 0, 0, 0, time.UTC)

func newStreamTestEnforcer(t *testing.T, policies ...permissions.PolicyRule) *permissions.Enforcer {
	t.Helper()
...
func streamFederationSub() string {
	return fmt.Sprintf("federation:%s", streamTestPeerID)
}

func newStreamTestHandler(t *testing.T, enforcer *permissions.Enforcer, index RecordingIndex) *StreamingHandler {
	t.Helper()
	h, err := NewStreamingHandler(StreamingHandlerConfig{
		Enforcer: enforcer,
		Index:    index,
		Cameras: &stubCameraRegistry{
			cameras: map[string]string{streamTestCamID: streamTestTenantID},
			urls:    map[string]string{streamTestCamID: streamTestRecURL},
			recIDs:  map[string]string{streamTestCamID: streamTestRecID},
		},
		Signer:  &stubSigner{token: "signed-test-token"},
		PeerID:  &stubPeerExtractor{id: streamTestPeerID},
...
// newStreamTestClient sets up an httptest server with the handler and returns a
// connect client pointed at it.
func newStreamTestClient(t *testing.T, h *StreamingHandler) (kaivuev1connect.FederationPeerServiceClient, func()) {
	t.Helper()
...
func TestSearchRecordings_WithGrants_ReturnsResults(t *testing.T) {
	enforcer := newStreamTestEnforcer(t,
		grantPolicy(streamFederationSub(), streamTestTenantID+"/recordings/*", permissions.ActionViewPlayback),
	)

	now := time.Now()
	entries := []RecordingEntry{
		{
			TenantID:    streamTestTenantID,
			RecorderID:  streamTestRecID,
			CameraID:    streamTestCamID,
			SegmentID:   "seg-1",
	...
	h := newStreamTestHandler(t, enforcer, &stubRecordingIndex{entries: entries})
	client, cleanup := newStreamTestClient(t, h)
	defer cleanup()

	stream, err := client.SearchRecordings(context.Background(), connect.NewRequest(&kaivuev1.SearchRecordingsRequest{
		Tenant:   streamTestTenant,
		PageSize: 50,
	}))
	if err != nil {
...
func TestSearchRecordings_WithoutGrants_PermissionDenied(t *testing.T) {
	enforcer := newStreamTestEnforcer(t) // no policies

	h := newStreamTestHandler(t, enforcer, &stubRecordingIndex{})
	client, cleanup := newStreamTestClient(t, h)
	defer cleanup()
...
func TestSearchRecordings_CameraFilterDenied(t *testing.T) {
	...
	stream, err := client.SearchRecordings(context.Background(), connect.NewRequest(&kaivuev1.SearchRecordingsRequest{
		Tenant:    streamTestTenant,
		CameraIds: []string{streamTestCamID},
	}))
	if err != nil {
		t.Fatalf("SearchRecordings: %v", err)
	}
...
func TestSearchRecordings_CameraFilterGranted(t *testing.T) {
	enforcer := newStreamTestEnforcer(t,
		grantPolicy(streamFederationSub(), streamTestTenantID+"/recordings/*", permissions.ActionViewPlayback),
		grantPolicy(streamFederationSub(), streamTestTenantID+"/cameras/"+streamTestCamID, permissions.ActionViewPlayback),
	)
...
func TestSearchRecordings_TimeRangePassedThrough(t *testing.T) {
	...
	h := newStreamTestHandler(t, enforcer, capIdx)
	client, cleanup := newStreamTestClient(t, h)
	defer cleanup()
...
func TestSearchRecordings_EmptyResults(t *testing.T) {
	...
	h := newStreamTestHandler(t, enforcer, &stubRecordingIndex{entries: nil})
	client, cleanup := newStreamTestClient(t, h)
	defer cleanup()
...
func TestMintStreamURL_WithGrants_ReturnsSignedURL(t *testing.T) {
	enforcer := newStreamTestEnforcer(t,
		grantPolicy(streamFederationSub(), streamTestTenantID+"/cameras/"+streamTestCamID, permissions.ActionViewLive),
	)
	...
	if !strings.HasPrefix(msg.Url, streamTestRecURL+"/webrtc/") {
	...
	if msg.Claims.CameraId != streamTestCamID {
		t.Errorf("Claims.CameraId = %q, want %q", msg.Claims.CameraId, streamTestCamID)
	}
	if msg.Claims.RecorderId != streamTestRecID {
		t.Errorf("Claims.RecorderId = %q, want %q", msg.Claims.RecorderId, streamTestRecID)
	}
	if msg.Claims.TenantRef == nil || msg.Claims.TenantRef.Id != streamTestTenantID {
		t.Errorf("Claims.TenantRef.Id = %q, want %q", msg.Claims.TenantRef.GetId(), streamTestTenantID)
	}
}
...
func TestMintStreamURL_UnknownCamera_NotFound(t *testing.T) {
	enforcer := newStreamTestEnforcer(t,
		grantPolicy(streamFederationSub(), streamTestTenantID+"/cameras/*", permissions.ActionViewLive),
	)
...
func TestMintStreamURL_ProtocolURLFormats(t *testing.T) {
	enforcer := newStreamTestEnforcer(t,
		grantPolicy(streamFederationSub(), streamTestTenantID+"/cameras/"+streamTestCamID, "*"),
	)
...
// --- NewStreamingHandler validation tests ------------------------------------

func TestNewStreamingHandler_MissingDeps(t *testing.T) {
	enforcer := newStreamTestEnforcer(t)
	base := StreamingHandlerConfig{
		Enforcer: enforcer,
		Index:    &stubRecordingIndex{},
	...
	tests := []struct {
		name string
		mut  func(*StreamingHandlerConfig)
		want string
	}{
		{"no enforcer", func(c *StreamingHandlerConfig) { c.Enforcer = nil }, "enforcer is required"},
		{"no index", func(c *StreamingHandlerConfig) { c.Index = nil }, "recording index is required"},
		{"no cameras", func(c *StreamingHandlerConfig) { c.Cameras = nil }, "camera registry is required"},
		{"no signer", func(c *StreamingHandlerConfig) { c.Signer = nil }, "stream signer is required"},
		{"no peerID", func(c *StreamingHandlerConfig) { c.PeerID = nil }, "peer identity extractor is required"},
	}
