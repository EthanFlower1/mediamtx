...
// GetTrackWithSightings returns a track with all its sightings.
// CameraName is populated from camera_id since this DB has no cameras table.
func (d *DB) GetTrackWithSightings(trackID int64) (*TrackWithSightings, error) {
	...
	rows, err := d.Query(`
		SELECT s.id, s.track_id, s.camera_id, s.camera_id,
