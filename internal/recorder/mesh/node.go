// Package mesh provides the Recorder-role convenience wrapper around
// internal/shared/mesh/tsnet. It exists so that Recorder callers
// never import the Directory's mesh package (and vice versa), in
...
// identifier to produce the mesh hostname for a Recorder node.
const RoleHostnamePrefix = "recorder-"

// Config is the Recorder-specific subset of tsnet.NodeConfig. The
// caller provides the component identifier (typically the Recorder
// UUID); this package prefixes it with "recorder-" to produce a
...
// New constructs a Recorder mesh node. The returned value is a
