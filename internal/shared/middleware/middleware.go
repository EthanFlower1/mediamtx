// Package middleware provides shared HTTP middleware for JWT validation.
package middleware
...
// SessionActivityUpdater is an optional interface for recording session
// activity. Pass a concrete *db.DB or a test double via the DB field.
type SessionActivityUpdater interface {
	UpdateSessionActivity(sessionID, ip string) error
}

// Middleware validates JWT tokens on incoming requests.
type Middleware struct {
	PrivateKey *rsa.PrivateKey
	DB         SessionActivityUpdater // optional; when set, updates session activity on each request
}
